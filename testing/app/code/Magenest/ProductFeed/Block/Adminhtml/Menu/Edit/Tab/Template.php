...
class Template extends Generic implements TabInterface
{
    ...
    protected function _prepareForm()
    {
        ...
        if ($template == 'google') {
            ...
            $setTitleFeed = null;
            if (!empty($model->getAttributeTemplate())) {
                $setTitleFeed = $model->setData('titlefeed',
                    json_decode($model->getAttributeTemplate(), true)['titlefeed']
                );
            }
            ...
            $setDescription = null;
            if (!empty($model->getAttributeTemplate())) {
                $setDescription = $model->setData('description',
                    json_decode($model->getAttributeTemplate(), true)['description']
                );
            }
            ...
            if (!empty($model->getAttributeTemplate()) &&
                isset(json_decode($model->getAttributeTemplate(), true)['fieldseparate'])
            ) {
                $setFieldSeparate = $model->setData('fieldseparate',
                    json_decode($model->getAttributeTemplate(), true)['fieldseparate']
                );
            }
            ...
            if (!empty($model->getAttributeTemplate()) &&
                isset(json_decode($model->getAttributeTemplate(), true)['fieldaroundby'])
            ) {
                $setFieldSeparate = $model->setData('fieldaroundby',
                    json_decode($model->getAttributeTemplate(), true)['fieldaroundby']
                );
            }
            ...
            if (!empty($model->getAttributeTemplate()) &&
                ...
                $setIncludeFieldHeader = $model->setData('includefieldheader',
                    json_decode($model->getAttributeTemplate(), true)['includefieldheader']);
            }
        }
