    ...
    protected function _processFieldMaps($fieldMaps)
    {
        ...
        foreach ($fieldMaps as $fieldMap) {
            if ($fieldMap['col_type'] == 'attribute') {
                $this->_fieldMaps = array_merge_recursive(
                    $this->_fieldMaps, [$fieldMap['col_attr_temp'] => $fieldMap['col_val']]
                );
            }
        }
...
<channel>';
        ...
        $headerTitle = $this->_headerFeed['titlefeed'] ?? "Google Shoppping Feed";
        $html .= $this->buildXml("title", $headerTitle, false);
        $html .= $this->buildXml("link", "{{ store.base_url }}", false);
        $headerDescription = $this->_headerFeed['description'] ?? "";
