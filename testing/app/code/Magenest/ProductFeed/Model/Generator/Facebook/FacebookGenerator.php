...
class FacebookGenerator
{
    ...
    public function __construct(
        ...
        $this->directory =
            $filesystem->getDirectoryWrite(\Magento\Framework\App\Filesystem\DirectoryList::PUB);
    }
    ...
    public function generateSegment($feed, $type)
    {
        ...
        return !empty($generate) ? $generate : '';
    }
    ...
    public function generateOnce($feed)
    {
        ...
        if ($fieldsMap == '') {
            ...
            $templateHtml =
                str_replace('}}', "| mpCorrect:'" . $modifyAroundBy . "','" . $modifySeparate . "'}}",
                    $templateHtml
                );
    ...
    protected function generateHeader($feed, $type)
    {
        ...
        $filepath = sprintf('%s%s_%s', self::FEED_FILE_PATH, self::TEMPLATE_HEADER, $name);
