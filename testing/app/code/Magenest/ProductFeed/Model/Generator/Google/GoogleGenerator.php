...
class GoogleGenerator
{
    ...
    protected function saveAllFile($feed)
    {
        ...
        $temp = '';
        $temp .= file_get_contents($pathHeaderFile) . '' .
            file_get_contents($pathItemsFile) . '' . file_get_contents($pathFooterFile);

        $fileUrl = self::FEED_FILE_PATH . '' . $fileName;

        file_put_contents($fileUrl, $temp);
