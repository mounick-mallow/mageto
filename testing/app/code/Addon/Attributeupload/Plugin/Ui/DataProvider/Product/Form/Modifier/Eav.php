...
namespace Addon\Attributeupload\Plugin\Ui\DataProvider\Product\Form\Modifier;
...
class Eav
{
     ...
     * @param mixed $attributesToDisable
     * @param mixed $attributesToEliminate
     */
    ...
    public function afterModifyData(
        ...
        foreach (array_keys($this->getGroups()) as $groupCode) {
            $attributeList = !empty($this->getAttributes()[$groupCode]) ?
                $this->getAttributes()[$groupCode] : [];

            foreach ($attributeList as $attribute) {
                if ($attribute->getFrontendInput() != 'file' ||
                    null == ($attributeValue = $this->setupAttributeData($attribute))
                ) {
                    continue;
                }
                ...
                try {
                    $path = $this->_filesystem->getDirectoryRead(DirectoryList::MEDIA)
                        ->getRelativePath('catalog/attribute-data/');

                    $fileSize = $this->_filesystem->getDirectoryRead(DirectoryList::MEDIA)
                        ->stat($path . $attributeValue);
                    ...
                    $size = array_key_exists(7, $fileSize) ? $fileSize[7] : 0;
                    $mediaBaseUrl = $this->urlBuilder->getBaseUrl([
                        '_type' => UrlInterface::URL_TYPE_MEDIA
                    ]);

                    $attributeDataLink = 'catalog/attribute-data/';
                    $url =  sprintf('%s%s%s', $mediaBaseUrl, $attributeDataLink, $attributeValue);
                ...
                $result[$productId][self::DATA_SOURCE_DEFAULT][$attribute->getAttributeCode()] =
                    $attributeValue;
            }
        }
    ...
    public function afterSetupAttributeMeta(
        ...
        if ('file' == $attribute->getFrontendInput()) {
            $otherInformationForFile = [
                ...
                'component' => 'Addon_Attributeupload/js/form/element/file-uploader',
                'elementTmpl' => 'Magento_Ui/js/form/element/file-uploader',
                'fileInputName' => 'file',
                'uploaderConfig' => [
                    'url' => $this->urlBuilder->getUrl('attributeupload/file/upload', [
                        'type' => 'file',
                        '_secure' => true
                    ])
                ],
            ...
            $result['arguments']['data']['config'] = array_merge(
    ...
    private function loadAttributes(AttributeGroupInterface $group)
    {
        ...
        foreach ($groupAttributes as $attribute) {
            ...
            if ($isRelated) {
                $loadedAttributeList[] = $attribute;
            }
        }

        return $loadedAttributeList;
    }
    ...
    public function setupAttributeData(ProductAttributeInterface $attribute)
    {
        ...
        $notUsed = !$prevSetId || ($prevSetId &&
            !in_array($attribute->getAttributeCode(), $this->getPreviousSetAttributes()));
    ...
    private function canDisplayUseDefault(ProductAttributeInterface $attribute)
    {
        ...
        return $this->canDisplayUseDefault[$attributeCode] = (
            ...
            && $product && $product->getId() && $product->getStoreId());
    }
