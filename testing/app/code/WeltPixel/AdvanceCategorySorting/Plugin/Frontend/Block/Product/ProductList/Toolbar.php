...
class Toolbar
{
                    ...
                    switch ($this->_getRealCurrentOrder($subject)) {
                        ...
                        case 'top_seller':
                            if (!$this->filtered) {
                                ...
                                $collection->getSelect()->joinLeft(
                                    $joinTableName,
                                    'e.entity_id = ' . $joinTableName . '.product_id AND '
                                    . $joinTableName . '.store_id=' . $this->_storeManager->getStore()->getId(),
                                    [
                                        $joinTableName . '.qty_ordered' =>
                                            'SUM(' . $joinTableName . '.qty_ordered) AS ordered'
                                    ]
                                )
                                    ->group('e.entity_id')
                        ...
                        case 'top_rated':
                            if (!$this->filtered) {
                                ...
                                $collection->getSelect()->joinLeft(
                                    $joinTableName,
                                    'e.entity_id = ' . $joinTableName . '.entity_pk_value AND '
                                    . $joinTableName . '.store_id=' . $this->_storeManager->getStore()->getId(),
                        ...
                        case 'most_reviewed':
                            if (!$this->filtered) {
                                ...
                                $collection->getSelect()->joinLeft(
                                    $joinTableName,
                                    'e.entity_id = ' . $joinTableName . '.entity_pk_value AND ' . $joinTableName
    ...
    public function aroundSetDefaultOrder(
        ...
        if ($this->_getRealCurrentOrder($subject) == 'relevance') {
            ...
            $sortingField = $currentSortOrder ? $currentSortOrder : $defaultSortOrder;
        }
        /**
         * concat direction if missing but not for 'relevance'
         */
        if (strpos($sortingField, '~') === false && $sortingField != 'relevance') {
            $direction = $this->_getRealCurrentDirection($subject);
            $sortingField .=  '~' . $direction;
        }

        return $proceed($sortingField);
    }
