...
class GenerateCssAfterStaticFilesCleanObserver implements ObserverInterface
{
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        $generateLessCommand = $objectManager->get(\WeltPixel\Command\Console\Command\GenerateLessCommand::class);
        $observerObject = $objectManager->get(\Magento\Framework\Event\Observer::class);
        $generationContainer = $generateLessCommand->getGenerationContainer();
        $errorMsg = [];

        foreach ($generationContainer as $key => $item) {
            try {
                $item->execute($observerObject);
            } catch (\Exception $ex) {
