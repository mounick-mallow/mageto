...
class BlockProductView
{
    ...
    public const XML_PATH_WELTPIXEL_PRODUCTPAGE_REMOVE_QTY = 'weltpixel_product_page/general/remove_qty_box';
    public const XML_PATH_WELTPIXEL_PRODUCTPAGE_REMOVE_EMAIL = 'weltpixel_product_page/general/remove_email';
    ...
    */
    public function afterShouldRenderQuantity(
        ...
        if (!$isProductSet && $this->request->getFullActionName() == 'catalog_product_view') {
            $removeQtySelector = $this->scopeConfig->getValue(
                self::XML_PATH_WELTPIXEL_PRODUCTPAGE_REMOVE_QTY,
    ...
    public function afterCanEmailToFriend(\Magento\Catalog\Block\Product\View $subject, $result)
    {
        if ($result && $this->request->getFullActionName() == 'catalog_product_view') {
            $removeEmail = $this->scopeConfig->getValue(
                self::XML_PATH_WELTPIXEL_PRODUCTPAGE_REMOVE_EMAIL,
                \Magento\Store\Model\ScopeInterface::SCOPE_STORE
            );
            if ($removeEmail) {
                $result = false;
