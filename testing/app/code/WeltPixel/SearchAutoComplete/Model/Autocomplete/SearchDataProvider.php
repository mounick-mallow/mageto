...
class SearchDataProvider implements DataProviderInterface
{
        ...
        if ($productIds) {
            ...
            foreach ($sorted as $product) {
                $image = $this->imageHelper->init($product, 'product_page_image_small')
                    ->resize($this->_helper->getWidthOfTheImage($storeId))->getUrl();
                $resultItem = $this->itemFactory->create([
                    'id' => $product->getId(),
                    ...
                    'special_price' => $this->priceCurrency->format(
                        $product->getPriceInfo()->getPrice('special_price')->getAmount()->getValue(),
                        false
                    ),
                    'final_price' => $this->priceCurrency->format(
    ...
    protected function searchProducts($query)
    {
        ...
        $termFilter = $this->filterBuilder->setField('search_term')
            ->setValue($query)
            ->setConditionType('like')
            ->create();
        $visibilityFilter = $this->filterBuilder->setField('visibility')
            ->setValue($this->_productVisibility->getVisibleInSearchIds())
            ->setConditionType('in')
            ->create();
