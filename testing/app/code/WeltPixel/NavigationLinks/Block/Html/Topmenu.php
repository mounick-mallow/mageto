    ...
    )
    {
        ...
        foreach ($children as $child) {
            ...
            if ($childLevel == 0 && $outermostClass) {
                $outermostClassCode = ' class="mycls ' . $outermostClass . '" ';
                $child->setClass($outermostClass);
            }

            if ($childLevel == 1){
                $extra = '<a' . $openInNewTab . ' href="' . $child->getUrl() . '" ' . 'class="m2cls"' . '><span style="font-size: 16px !important; text-transform: capitalize !important; border-bottom: none !important; color: #575757 !important;">All ' . $this->escapeHtml(
                    $child->getName()
                ) . '</span></a>';
            }
...
//            if ($colBrakes && count($colBrakes) && $colBrakes[$counter]['colbrake']) {
		...
		$html .= '<div class="border-container top-menu-border">
    <div class="border-tl"></div>
    <div class="border-br"></div>
    </div><li class="megamenu level-top-sectioned  level0 nav-8 category-item last level-top ui-menu-item" data-test="brand" role="presentation"><a href="'.$this->getUrl("brands").'" class="level-top ui-corner-all" id="ui-id-9" tabindex="-1" role="menuitem"><span>'.$this->escapeHtml("Brands").'</span></a></li>';
