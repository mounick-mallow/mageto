...
class CmsBlockLoadAfter implements ObserverInterface
{
    ...
    public function execute(Observer $observer){
        ...
        if($globalCss = trim((string)$model->getCssGlobal())){
            $html .= $globalCss;
        }
        if($smallPhoneCss = trim((string)$model->getCssPhoneSmall())){
            $html .= ' @media screen and (min-width: '.$smallMobileBkp.'){'.$smallPhoneCss.'}';
        }
        if($phoneCss = trim((string)$model->getCssPhone())){
            $html .= ' @media (min-width: '.$mobileBkp.'){'.$phoneCss.'}';
        }
        if($smallTabletCss = trim((string)$model->getCssTabletSmall())){
            $html .= ' @media (min-width: '.$smallTabletBkp.'){'.$smallTabletCss.'}';
        }
        if($tabletCss = trim((string)$model->getCssTablet())){
            $html .= ' @media (min-width: '.$tabletBkp.'){'.$tabletCss.'}';
        }
        if($deskCss = trim((string)$model->getCssDesktop())){
            $html .= ' @media (min-width: '.$deskBkp.'){'.$deskCss.'}';
        }
        if($largeDeskCss = trim((string)$model->getCssDesktopLarge())){
        ...
        if(strpos((string)$model->getContent(),$html) === false){
