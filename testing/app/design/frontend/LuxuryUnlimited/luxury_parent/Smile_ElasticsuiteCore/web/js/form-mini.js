...
define([
    ...
    $.widget('smileEs.quickSearch', $.mage.quickSearch, {
        ...
        /**
         * Get the brand from the product id
         *
         * @param productId
         *
         * @returns {*|jQuery|HTMLElement}
         ...
         * @private
         */
        _getBrand: function (brandUrl,productId) {
            var result = 0;
            $.ajax({
                url: brandUrl,
        ...
        _getSectionTitle: function(type, data) {
            ...
            if (type === 'product') {
                title = $('<div class="autocomplete-list-title">' +
                        '<p class="title-' + type + '">Products list (' +
                            data[0]['num_results'] +
                            ')' +
                        '</p>' +
                        '<button type="submit" class="link">View all</button>' +
                    '</div>');
            }
            ...
            if (value.trim().length >= parseInt(this.options.minSearchLength, 10)) {
                ...
                this.currentRequest = $.ajax({
                    ...
                    success: $.proxy(function (data) {
                        ...
                        content.append('<div class="title-result"><span>Search results</span></div>');
                        ...
                        $.each(data, function(index, element) {
                            ...
                            var productDetailsHtml = $('<div>', {
                                class: 'product-details'
                            });


                            sectionDropdown.append(elementHtml);
                            image = '<div class= "es-img-div"><img class="es-product" src="'+element.image+'" /></div>';
                            price = element.price;
                            // sectionDropdown.append(elementHtml);
                            title = element.title;
                            type = element.type;
                            ...
                            var doc = new DOMParser().parseFromString(price, 'text/html');
                            var productIdElement = doc.querySelector('div[data-product-id]');
                            if(productIdElement) {
                                productId = productIdElement.getAttribute('data-product-id');
                                if(productId) {
                                    brandUrl = this.options.url;
                                    baseUrl = brandUrl.replace('search/ajax/suggest/','');
                                    brandUrl = baseUrl + 'smile/brand/brandname/productId/'+productId+'/';
                                }

                            }
                            if (type == 'term') {
                                title = '<div class="es-title-div"><span class="es-title es-product-name">'+element.title+'</span></div>';
                                sectionDropdown.append(title);
                            } else {
                                if(type =='product') {
                                    title = '<div class="es-title-div"><span class="es-title"><a href="'+element.url+'">'+element.title+'</a></span></div>';
                                    brand = '<div class="es-brand-div"><span class="es-brand" id="brand-'+productId+'"></span></div>';

                                    productDetailsHtml.append(brand);
                                    productDetailsHtml.append(title);
                                    productDetailsHtml.append(price);
                                    elementHtml.append(image);
                                    elementHtml.append(productDetailsHtml);
                                    sectionDropdown.append(elementHtml);
                                    brand = this._getBrand(brandUrl,productId);
