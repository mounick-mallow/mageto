    ...
    $.widget('magepow.ajaxcart', {
        ...
        _initAjaxcart: function () {
            ...
            self.element.off('click').on("click", options.addToCartButtonSelector, function(e){
                var confbuyNow = false;
                if($(this).hasClass('confbuynow')){
                    confbuyNow =true;
                }
                ...
                if (form) {
                    ...
                    if (isValid) {
                        ...
                        if ($.isNumeric(id)) {
                            ...
                            if (options.quickview) {
                                window.parent.ajaxCart._sendAjax(options.addUrl, data, oldAction, confbuyNow, form);
                                return false;
                            }

                            self._sendAjax(options.addUrl, data, oldAction,confbuyNow, form);
                            return false;
                        }
                    ...
                    if (dataPost) {
                        ...
                        self._sendAjax(options.addUrl, data, oldAction,confbuyNow);
                        return false;
                    } else {
        ...
        _sendAjax: function (addUrl, data, oldAction, confbuyNow, form = false) {
            var body = $('body');
            var options = this.options;
            var self = this;
            if(form){
                ...
                data.append("confbuyNow",confbuyNow);
            }
            $.ajax({
                type: 'post',
                url: addUrl,
                ...
                success: function (data) {
                    //console.dir(data);
                    var visitedVal = $(form).closest('#product_addtocart_form').find("input[name='visited']").val();
                    if (data.popup) {
                        if (visitedVal) {
                        ...
                        if (data.error) {
                            ...
                            $('.content-ajaxcart').find('.viewcart').text($.mage.__('View Cart'));
                            $('.content-ajaxcart').find('.checkout-buy-now').hide();
                            $('.content-ajaxcart').find('.checkout-link').hide();
                        } else {
                            ...
                            $('.content-ajaxcart').find('h1').text($.mage.__('Product is Added to Cart'));
                        }
                    ...
                    if(form) self.enableAddToCartButton(form,confbuyNow);
                },
                error: function () {
                    window.location.href = oldAction;
                }
        ...
        _closePopup: function(){
            ...
            $body.find('.modal-ajaxcart').remove();
         ...
         * @param {String} form
         */
        enableAddToCartButton: function (form,confbuyNow) {
            var buttonText ='Added';
            if(confbuyNow)
                buttonText = 'Buy Now';
            var addToCartButtonTextAdded = this.options.addToCartButtonTextAdded || $t(buttonText),
                self = this,
                addToCartButton = $(form).find(this.options.addToCartButtonSelector);
            ...
            setTimeout(function () {
                var buttonTextTimeOut ='Add to Cart';
                if(confbuyNow)
                    buttonTextTimeOut = 'Buy Now';
                var addToCartButtonTextDefault = self.options.addToCartButtonTextDefault || $t(buttonTextTimeOut);
